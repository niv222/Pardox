...
def main ():
    ...
    #player2 = DQN_Agent(player=-1, env=env, train=True)   #0.1
    player2 = Random_Agent(player = 1, env=env)   
    #player2 = AlphaBeta_Agent(player=-1, environment=env)
    ...
    best_random = -200 #max(random_results)
    start_epoch = 0#results_file['epoch'] + 1
    max_step = 1000
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while not env.is_end_of_game(state_1):
            ...
            after_state_1 = env.get_next_state(action=action_1, state=state_1)
            step += 1
            reward_1, end_of_game_1 = env.reward(after_state_1, IsBlack=True)
            ...
            step += 1
            after_state_2 = env.get_next_state(action=action_2 ,state=state_2)
            after_state_2.player = 1
            reward_2, end_of_game_2 = env.reward(state=after_state_2, IsBlack=True)
        ...
        print (f'epoch={epoch} loss={loss:.5f}  avgloss={avgLoss:.5f} step={step}' ,end=" ")
